```python
"""
Blackjack (21) using OOP.

Classes:
Card: a card from the range of Ace to King with one of four suits
CardGroup: a collection of cards
CardDeck: a unique collection of 52 cards, which can be shuffled
Player: has a hand, which is a CardGroup, and asks the user for a choice
Robot: a special case of player where the choice is made automatically
Dealer: contains a CardDeck and distributes cards to the players

Functions:
playGame(): is a function which interacts with the classes to create a text based blackjack game.
